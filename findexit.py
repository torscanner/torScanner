...
def run(ADDRESSES_OF_INTEREST, varlibtor):
    global VERBOSE
    global INVERSE
...
""" % sys.argv[0]
    ...
    published = 0
    fp = ""
    files=open(varlibtor, "r")
    linesfile=files.readlines()
    print " ( lines in exitlist", len(linesfile), ")",
    ...
    ents = []
    if VERBOSE:
        ents = uniq_sort([ "%s:%s:FUFFA"%(s.ip,s.name) for s in printlist ])
    else:
